...
import { Client, ClientContact, SavedProposal, PIPELINE_STAGES, generateId } from '../../types';
        ...
        <div className="flex items-center gap-2">
          {editing ? (
            ...
            <>
              ...
              <button onClick={() => downloadProposalPdf(proposal, { suiteName: 'Prof G', clientName: proposal.briefing?.clientName, clientCompany: proposal.briefing?.company })} className="btn-secondary text-xs py-1.5">
                ↓ Download PDF
              </button>
...
export default function ClientModal({
  ...
  const [tab, setTab] = useState<'details' | 'contacts' | 'proposals'>('details');
      ...
      {viewingProposal && (
        <ProposalViewer
          ...
          onUpdate={onUpdateProposal ? (updates) => {
            ...
            setViewingProposal(prev => prev ? { ...prev, ...updates } : null);
          } : undefined}
        />
      )}
      {showImport && client && (
          ...
          {client && (
            <div className="flex border-b border-brand-cream">
              ...
              >
                Proposals
                {hasProposals && (
                  <span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-[#15803d] text-white text-xs font-bold">
                    {proposals.length}
                  </span>
                )}
              </button>
              ...
              <div className="px-6 py-5 space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[10px] text-[#15803d]/60 font-medium">Granola / meeting notes</span>
                  </div>
                  <textarea
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {(form.contacts ?? []).length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-xs text-brand-dark/35">Add contacts from this brand — decision makers, champions, influencers.</p>
                  </div>
                )}
                  ...
                  {[...proposals].reverse().map(p => (
                    <div key={p.id} className="card p-4 flex items-start gap-3">
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        ...
                        <button onClick={() => downloadProposalPdf(p, { suiteName: 'Prof G', clientName: form.name || p.briefing?.clientName, clientCompany: form.company || p.briefing?.company })} className="btn-secondary py-1 px-2.5 text-xs">↓ PDF</button>
