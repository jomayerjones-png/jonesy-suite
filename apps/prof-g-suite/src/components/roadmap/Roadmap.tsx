...
const WEEKS_INIT = [
  ...
  { id: 8, label: 'W/C 4 May', theme: 'Production Sprint', phase: 'may' as const, milestones: ['Production window opens — 6 weeks to 22 Jun', 'Large campaigns briefed and kicked off', 'Content execution begins'] },
  ...
  { id: 10, label: 'W/C 18 May', theme: 'Production Sprint', phase: 'may' as const, milestones: ['Final approvals sought', 'Publish-ready materials signed off', 'Campaigns can stretch into next cycle if needed'] },
  { id: 11, label: 'W/C 25 May', theme: 'Launch Prep', phase: 'june' as const, milestones: ['Press and distribution prep', 'Launch comms ready', 'Sponsor placements confirmed'] },
];
...
const CLOSERS_INIT = [
  { id: 1, category: 'PR & Marketing', items: ['PR and marketing plans confirmed', 'Partner visibility commitments defined', 'Editorial calendar shared with key partners'] },
  { id: 2, category: 'Launch Event & Activations', items: ['Launch event format confirmed', 'Activation options presented to partners', 'Partner presence at launch event defined'] },
  { id: 3, category: 'Content & Programming', items: ['Decision on next content themes', 'Forward-planning commitments offered to partners', 'Multi-cycle packages available for discussion'] },
...
export default function Roadmap({ companyName: _companyName }: { companyName: string }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8">
      ...
      {/* Print-only Prof G header */}
      <div className="print-only hidden items-center justify-between mb-6 pb-4 border-b-2 border-gray-200">
        <div style={{ backgroundColor: '#15803d', padding: '5px 12px', display: 'inline-block' }}>
          <span style={{ fontFamily: 'Georgia, serif', fontWeight: 700, color: '#FFFFFF', fontSize: '22px', letterSpacing: '-1px', lineHeight: 1 }}>Prof G</span>
        </div>
      ...
      <div className="mb-7">
        ...
        {subView === 'roadmap' && (
          <p className="mt-1.5 text-sm text-gray-500">
            Deals closed by 1 May · Launch 22 June
          </p>
        )}
        {subView === 'close' && (
          <p className="mt-1.5 text-sm text-gray-500">
        ...
        <>
          ...
          <div className="mt-7 bg-brand-gold rounded-lg px-6 py-5 flex items-center justify-between">
            <div>
              ...
              <div className="text-xl font-bold text-white tracking-tight">22 June 2026 — Launch day</div>
            </div>
            <span className="text-2xl text-white/30">✦</span>
          </div>

          <div className="mt-2.5 px-3.5 py-2.5 bg-white border border-gray-200 rounded-md text-xs text-gray-400">
            Custom programmes require 6 weeks production. Deals closed by 1 May enables delivery for launch. Larger campaigns can stretch into the next cycle.
          </div>
        </>
      )}
