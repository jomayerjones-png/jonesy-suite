...
function App() {
  ...
  const [companyName, setCompanyName] = useState<string>(() => {
    return localStorage.getItem(STORAGE_KEY_COMPANY) ?? 'Prof G';
  });
    ...
    <div className="min-h-screen bg-brand-light flex flex-col">
      ...
      <footer className="no-print bg-white border-t border-brand-cream px-6 py-2 flex items-center justify-between">
        <p className="text-xs text-brand-dark/35 font-medium">
          CONFIDENTIAL — Property of Prof G Media. This tool and all information contained within is strictly private and confidential. Unauthorised access, use, or distribution is prohibited.
        </p>
        <p className="text-xs text-brand-dark/25 flex-shrink-0 ml-6">© {new Date().getFullYear()} Prof G Media</p>
      </footer>
    </div>
  );
}
