...
const REPORT_API_KEY_STORAGE = 'status_suite_intel_api_key';
...
export default function WeeklyReport({ clients, companyName }: WeeklyReportProps) {
  ...
  const handleGenerateReport = async () => {
    ...
    const prompt = `You are writing a weekly partnership update report for ${companyName || 'our company'}. Status is a premium media brand covering the intersection of tech, business, and culture.
            ...
            {!showArchives && (
              <>
                <button
                  ...
                  className="px-3 py-1.5 text-sm font-semibold rounded-lg bg-[#E8471C] text-white hover:bg-[#d43d16] disabled:opacity-40 disabled:cursor-not-allowed transition-all flex items-center gap-1.5"
                >
        ...
        {showArchives ? (
          ...
          <div className="flex-1 overflow-auto p-5">
            <div ref={reportRef} className="max-w-4xl mx-auto space-y-4 print-compact">
              ...
              {aiGenerating && (
                <div className="no-print bg-[#E8471C]/5 border border-[#E8471C]/20 rounded-xl px-4 py-3">
                  <p className="text-xs text-[#E8471C] font-medium animate-pulse">Analyzing pipeline and generating report…</p>
                </div>
              )}
