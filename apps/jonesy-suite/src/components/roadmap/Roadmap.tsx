...
const ROADMAP_STORAGE_KEY = 'jonesy_suite_roadmap';

// ── Data ─────────────────────────────────────────────────────────
const WEEKS_INIT = [
  { id: 1, label: 'W/C 16 Mar', theme: 'Foundations', phase: 'now' as const, milestones: ['Audit and diagnostics complete', 'Strategic positioning defined', 'Deliverables scoped', 'Pipeline seeded'] },
  { id: 2, label: 'W/C 23 Mar', theme: 'Market', phase: 'now' as const, milestones: ['Discovery meetings underway', 'Market feedback gathered', 'Pipeline developed further'] },
  ...
  { id: 7, label: 'W/C 28 Apr', theme: 'Close', phase: 'may' as const, milestones: ['Final negotiations', 'Agreements confirmed', 'Engagements signed by 1 May'] },
  { id: 8, label: 'W/C 4 May', theme: 'Delivery Sprint', phase: 'may' as const, milestones: ['Delivery window opens — 6 weeks to 22 Jun', 'Workstreams briefed and kicked off', 'CRO execution begins'] },
  { id: 9, label: 'W/C 11 May', theme: 'Delivery Sprint', phase: 'may' as const, milestones: ['Deliverables in progress', 'Client reviews scheduled', 'Engagement delivery tracked'] },
  { id: 10, label: 'W/C 18 May', theme: 'Delivery Sprint', phase: 'may' as const, milestones: ['Final approvals sought', 'Client-ready materials signed off', 'Engagements can extend into next quarter if needed'] },
  { id: 11, label: 'W/C 25 May', theme: 'Launch Prep', phase: 'june' as const, milestones: ['Results compiled and reviewed', 'Next-phase recommendations ready', 'Ongoing retainer discussions initiated'] },
];

const GROUPS: Record<number, { label: string }> = {
  1: { label: 'March — Foundations' },
  3: { label: 'April — Proposals & Pipeline' },
  6: { label: 'Key Meeting, 23 April' },
  7: { label: 'May — Deals Close · Delivery Sprint Begins' },
  11: { label: 'June — Launch Prep' },
};

const CLOSERS_INIT = [
  { id: 1, category: 'Strategy & Positioning', items: ['CRO strategy decks finalised', 'Client-specific positioning confirmed', 'Competitive analysis shared with key prospects'] },
  { id: 2, category: 'Pipeline & Outreach', items: ['Outreach cadence confirmed', 'Prospect qualification criteria agreed', 'Pipeline targets set and tracked'] },
  { id: 3, category: 'Engagement Scoping', items: ['Scope of work templates finalised', 'Pricing and packaging approved', 'Multi-phase engagement options defined'] },
  { id: 4, category: 'Client Activation Framework', items: ['Post-sale onboarding process agreed', 'Delivery ownership and contacts confirmed', 'Reporting and measurement approach aligned'] },
];
...
export default function Roadmap({ companyName: _companyName }: { companyName: string }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8">
      ...
      {/* Print-only Jonesy&Co header */}
      <div className="print-only hidden items-center justify-between mb-6 pb-4 border-b-2 border-gray-200">
        <div style={{ backgroundColor: '#111827', padding: '5px 12px', display: 'inline-block' }}>
          <span style={{ fontFamily: 'Georgia, serif', fontWeight: 700, color: '#FFFFFF', fontSize: '22px', letterSpacing: '-1px', lineHeight: 1 }}>Jonesy&Co</span>
        </div>
      ...
      <div className="mb-7">
        ...
        {subView === 'roadmap' && (
          <p className="mt-1.5 text-sm text-gray-500">
            Engagements signed by 1 May · Delivery complete 22 June
          </p>
        )}
        {subView === 'close' && (
          <p className="mt-1.5 text-sm text-gray-500">
            Gate: <strong className="text-brand-gold">1 May 2026</strong> · All of the below must be resolved before engagements are signed.
          </p>
        )}
      </div>
        ...
        <>
          ...
          <div className="mt-7 bg-brand-gold rounded-lg px-6 py-5 flex items-center justify-between">
            <div>
              ...
              <div className="text-xl font-bold text-white tracking-tight">22 June 2026 — All deliverables shipped</div>
            </div>
            <span className="text-2xl text-white/30">✦</span>
          </div>

          <div className="mt-2.5 px-3.5 py-2.5 bg-white border border-gray-200 rounded-md text-xs text-gray-400">
            Custom engagements require 6 weeks delivery. Deals signed by 1 May enables completion by 22 June. Larger programmes can extend into the next quarter.
          </div>
        </>
      )}
