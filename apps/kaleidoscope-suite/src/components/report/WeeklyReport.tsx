...
const STORAGE_KEY_PREV_SNAPSHOT = 'kaleidoscope_suite_prev_snapshot';
const REPORT_API_KEY_STORAGE = 'kaleidoscope_suite_intel_api_key';
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <p className="text-white/40 text-xs font-medium uppercase tracking-widest">Partnership Update</p>
                  </div>
                    ...
                    {archive.stats.deals.map((deal, i) => {
                      ...
                      return (
                        <tr key={i} className={`border-b border-brand-cream/50 ${deal.stale ? 'bg-amber-50' : ''}`}>
                          ...
                          <td className="py-2 pr-3 text-right font-bold text-[#7C3AED]">{formatCurrency(deal.value)}</td>
...
export default function WeeklyReport({ clients, companyName }: WeeklyReportProps) {
  ...
  const handleGenerateReport = async () => {
    ...
    const prompt = `You are writing a weekly partnership update report for ${companyName || 'our company'}. Kaleidoscope is a premium podcast studio and iHeart's flagship science and technology network.
...
{"stageChanges":"line1\\nline2","activity":"line1\\nline2","materials":"line1\\nline2","focusAhead":"line1\\nline2"}`;
    ...
    try {
      ...
      else { const s = text.indexOf('{'), e = text.lastIndexOf('}'); if (s !== -1 && e > s) jsonStr = text.slice(s, e + 1); }
      if (!jsonStr) throw new Error('Could not parse AI response — try again.');
      const cleaned = jsonStr.replace(/,\s*([}\]])/g, '$1').replace(/[\x00-\x1f]/g, ch => ch === '\n' ? '\\n' : ch === '\t' ? '\\t' : '');
  ...
  const stats = useMemo(() => {
    ...
    const byStage: Record<PipelineStage, Client[]> = {
      Prospect: [], Engaged: [], 'Meeting Set': [], 'Proposal Sent': [], 'Revised Proposal Sent': [], Feedback: [], Close: [],
    };
  ...
  const currentSnapshot: PipelineSnapshot = useMemo(() => {
    const byStage: Record<PipelineStage, { count: number; value: number }> = {
      ...
      'Proposal Sent': { count: 0, value: 0 }, 'Revised Proposal Sent': { count: 0, value: 0 }, Feedback: { count: 0, value: 0 }, Close: { count: 0, value: 0 },
    };
    PIPELINE_STAGES.forEach(s => {
      byStage[s] = {
        count: stats.byStage[s]?.length ?? 0,
        value: stats.byStage[s]?.reduce((sum, c) => sum + c.value, 0) ?? 0,
      };
    });
    return {
  ...
  const maxCount = Math.max(...PIPELINE_STAGES.map(s => stats.byStage[s]?.length ?? 0), 1);
            ...
            {!showArchives && (
              <>
                <button
                  ...
                  className="px-3 py-1.5 text-sm font-semibold rounded-lg bg-[#7C3AED] text-white hover:bg-[#6D28D9] disabled:opacity-40 disabled:cursor-not-allowed transition-all flex items-center gap-1.5"
                >
        ...
        {showArchives ? (
          ...
          <div className="flex-1 overflow-auto p-5">
            <div ref={reportRef} className="max-w-4xl mx-auto space-y-4 print-compact">
              ...
              {aiGenerating && (
                <div className="no-print bg-[#7C3AED]/5 border border-[#7C3AED]/20 rounded-xl px-4 py-3">
                  <p className="text-xs text-[#7C3AED] font-medium animate-pulse">Analyzing pipeline and generating report…</p>
                </div>
              )}
              ...
              <EditableSection
                title="Stage Changes"
                placeholder={`e.g.\nNetflix moved from Proposal Sent → Feedback — awaiting CMO sign-off\nGoogle advanced to Meeting Set — call confirmed for next week\nAmazon closed at $150K — 6-month activation deal`}
              ...
              <EditableSection
                ...
                placeholder={`e.g.\nCall with Lisa Kim at Netflix — reviewed partnership scope (Tue)\nProposal sent to Google — podcast sponsorship, $200K (Wed)\nIntro meeting with David Park, Amazon Studios (Thu)`}
                        ...
                        {stats.sortedDeals.map(client => {
                          ...
                          return (
                            <tr key={client.id} className={`border-b border-brand-cream/50 ${stale ? 'bg-amber-50' : ''}`}>
                              ...
                              <td className="py-2 pr-3 text-right font-bold text-[#7C3AED]">{formatCurrency(client.value)}</td>
              ...
              <EditableSection
                title="New Materials"
                placeholder={`e.g.\nNetflix x Kaleidoscope Custom Partnership deck (approved)\nGoogle Podcast Sponsorship proposal (draft)\nUpdated media kit with Q1 performance data`}
              ...
              <EditableSection
                ...
                placeholder={`e.g.\nClose Netflix partnership — final sign-off expected next week\nSecond meeting with Google — present sponsorship terms\nInitiate Amazon event co-branding conversation`}
