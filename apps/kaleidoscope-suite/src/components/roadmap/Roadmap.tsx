...
function Editable({ value, onChange, placeholder, className = '' }: { value: string; onChange: (v: string) => void; placeholder?: string; className?: string }) {
  ...
  if (on) return (
    <input
      ...
      className={`border-b-2 border-[#7C3AED] outline-none bg-transparent font-inherit w-full ${className}`}
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8">
      ...
      <div className="print-only hidden items-center justify-between mb-6 pb-4 border-b-2 border-gray-200">
        <div style={{ backgroundColor: '#7C3AED', padding: '5px 12px', display: 'inline-block' }}>
          <span style={{ fontFamily: 'monospace', fontWeight: 700, color: '#fff', fontSize: '16px', letterSpacing: '-0.5px', lineHeight: 1 }}>K⟡</span>
        </div>
        <div style={{ textAlign: 'right', fontSize: '9pt', color: '#888' }}>
          <p style={{ margin: 0, fontWeight: 600 }}>Kaleidoscope · {qInfo.label} Roadmap</p>
      ...
      <div className="mb-7">
        <div className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1.5">
          Kaleidoscope · {qInfo.label} Roadmap
        </div>
            ...
            <div className="flex items-stretch bg-white border border-gray-200 rounded-lg overflow-hidden">
              ...
              <div className="flex-1 px-4 py-3.5 flex flex-wrap items-center gap-x-6 gap-y-2">
                ...
                {week.milestones.map((m, mi) => (
                  <div key={mi} className="flex items-center gap-2">
                    ...
                    <span
                      ...
                      className="no-print text-gray-200 hover:text-[#7C3AED] cursor-pointer text-[10px] select-none transition-colors"
                    >
                      ✕
                    </span>
                  </div>
                ...
                <span
                  ...
                  className="no-print text-[11px] text-gray-300 hover:text-[#7C3AED] cursor-pointer select-none transition-colors"
                >
                  + add
                </span>
              </div>
